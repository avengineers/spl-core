...
class TestDocumentation(SplKickstartProjectIntegrationTestBase):
    def test_build_reports(self) -> None:
        ...
        for component_path in self.spl_project.components:
            ...
            if len(list(self.spl_project.artifacts.project_root_dir.joinpath(component_path).glob("test/*"))):
                ...
                # Coverage Markdown content validation — check rendered table structure and links
                self._assert_coverage_table_content(reports_dir / "coverage.html", reports_dir)
    ...
    @staticmethod
    def _assert_coverage_table_content(coverage_html: Path, reports_dir: Path) -> None:
        """Assert that the coverage page contains a proper table with links to detail pages.

        The coverage.md generated by coverage_to_md.py is rendered by Sphinx into this HTML.
        Expected columns: File, Lines, Covered, Missed, Coverage/Line %.
        Each source file should ideally have a clickable link to a gcovr HTML detail page,
        but link generation depends on correct --html-dir path resolution at build time.
        """
        ...
        # Find the coverage table by looking for expected column names
        coverage_table = None
        for table in soup.find_all("table"):
            headers = [th.get_text(strip=True) for th in table.find_all("th")]
            if any("File" in h for h in headers) and any("Covered" in h or "Coverage" in h for h in headers):
                coverage_table = table
                break

        assert coverage_table is not None, f"No coverage table with expected columns found in {coverage_html.name}"

        headers = [th.get_text(strip=True) for th in coverage_table.find_all("th")]

        # Verify expected columns from coverage_to_md.py output
        # Without branches: "File", "Lines", "Coverage"
        # With branches: additionally "Branches", "Branch %"
        for expected in ["File", "Lines"]:
            assert any(expected in h for h in headers), f"Column '{expected}' not found. Headers: {headers}"
        # At least one coverage percentage column must be present
        assert any("Coverage" in h or "Line %" in h for h in headers), f"No coverage percentage column found. Headers: {headers}"

        # Verify data rows: at least one file row plus TOTAL
        data_rows = [r for r in coverage_table.find_all("tr") if r.find("td")]
        assert len(data_rows) >= 2, f"Expected at least 2 data rows (files + TOTAL), found {len(data_rows)}"

        # Links to detail pages: at least one file should have a clickable link
        links = coverage_table.find_all("a")
        assert len(links) >= 1, "Coverage table has no links to detail pages — check _find_detail_file matching"
        for link in links:
            href = link.get("href", "")
            if href.startswith("coverage/"):
                detail_path = reports_dir / href
                assert detail_path.exists(), f"Linked coverage detail page not found: {detail_path}"
