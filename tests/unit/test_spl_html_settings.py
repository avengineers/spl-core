...
class TestSplHtmlSettings:
    ...
    def test_copyright_default(self):
        """Default copyright uses the current year and names a holder (brand-agnostic)."""
        import datetime

        year = datetime.date.today().year
        assert spl_html_settings.copyright.startswith(f"{year}, ")
        holder = spl_html_settings.copyright.removeprefix(f"{year}, ").strip()
        assert holder, "Default copyright must name a holder"
